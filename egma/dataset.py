...
from .prompts import process_class_prompts, process_class_prompts_for_tuning, process_class_prompts_openclip
from .prompts import generate_chexpert_class_prompts
from . import constants

from.open_clip.tokenizer import tokenize

LOCAL_DATA_DIR = osp.join(osp.dirname(osp.dirname(__file__)), 'local_data')
SENTENCE_LABEL_PATH = osp.join(LOCAL_DATA_DIR, 'sentence-label.csv')
SENTENCE_LABEL2_PATH = osp.join(LOCAL_DATA_DIR, 'sentence-label2.csv')
...
class ImageTextContrastiveDataset(Dataset):
    ...
    def __init__(self, datalist=['mimic-cxr-train', 'chexpert-train'], imgtransform=None) -> None:
        ...
        self.sentence_label = pd.read_csv(SENTENCE_LABEL_PATH, index_col=0).fillna(0)
        print(f'load sentence prompts from {SENTENCE_LABEL_PATH}')
...
class my_Hyrachy_ImageTextContrastiveDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        hm_img = cv2.imread(".....Precessed/all_static_heatmaps/{}.jpg".format(img_id), 0)
...
class my_Hyrachy_1083AUG_ImageTextContrastiveDataset_RandomSentenceHeatmap(Dataset):
    ...
    def __init__(self, datapath, imgtransform=None, hm_transform=None) -> None:
        ...
        #     df_list.append(df)
        self.df = pd.read_csv(datapath)
        self.mimic_eye_df = pd.read_csv(r"..../mimic_eye_pair_data_v1.csv")
        ...
        self.hm_transform = hm_transform
        # use labeled sentences as prompts for chexpert training
        self.sentence_label = pd.read_csv(SENTENCE_LABEL2_PATH, index_col=0).fillna(0)
        print(f'load sentence prompts from {SENTENCE_LABEL2_PATH}')
        self._preprocess_sentence_label()
        self._build_prompt_sentence()
...
class my_PartGaze_OpenCLIP_ImageTextContrastiveCollator:
    def __init__(self, use_eda=True):
        '''Args:
        use_EDA: easy data augmentation from textaugment
        '''
    ...
    def __call__(self, batch):
        ...
        for batch_i, data in enumerate(batch):
            ...
            inputs['img_labels'].append(data[2])
            inputs['text_labels'].append(data[3])
        text_inputs = self.tokenizer(report_list)
        inputs['pixel_values'] = torch.cat(inputs['pixel_values'], 0)
        inputs['gaze_hm'] = torch.cat(inputs['gaze_hm'], 0)
        if inputs['pixel_values'].shape[1] == 1: inputs['pixel_values'] = inputs['pixel_values'].repeat((1,3,1,1))
        inputs['img_labels'] = torch.tensor(np.stack(inputs['img_labels']).astype(float))
...
class my_FILIP_OpenI_ImageTextContrastiveDataset_AllSentenceHeatmap(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgpath = osp.join(r"..../NLMCXR_png/", img_name)
...
class ZeroShotImageDataset(Dataset):
    def __init__(self,
        ...
        for data in datalist:
            # filename = f'./local_data/{data}-meta.csv'
            filename = f'..../EGMA/local_data/{data}-meta.csv'
            print('load data from', filename)
            df = pd.read_csv(filename, index_col=0)
            df_list.append(df)
        self.df = pd.concat(df_list, axis=0).reset_index(drop=True)
...
class my_ZeroShotImageDataset(Dataset):
    def __init__(self,
                 datapath=f'..../CheXpert-v1.0/chexpert_5x200.csv',
                 class_names=None,
                 imgtransform=None,
                 ) -> None:
        '''support data list in mimic-5x200, chexpert-5x200, rsna-balanced-test, covid-test
...
class my_ZeroShotImageCollator_OpenCLIP:
    def __init__(self, mode, cls_prompts=None, n_prompt=5):
        # initialize tokenizer
        self.tokenizer = tokenize
        # self.tokenizer.model_max_length = 77
        ...
        # process cls prompts into texts indices
        self.prompt_texts_inputs = process_class_prompts_openclip(self.cls_prompts, self.tokenizer)
...
class my_SIIMACR_ZeroShotImageDataset(Dataset):
    def __init__(self,
                 datapath=f'..../siim_acr_zeroshot_v1.csv',
                 class_names=None,
                 imgtransform=None,
                 ) -> None:
        super().__init__()
...
class my_RSNA_ZeroShotImageDataset(Dataset):
    def __init__(self,
                 datapath=f'....RSNA_Stage2_training_files/test_jpg_split.csv',
                 class_names=None,
                 imgtransform=None,
                 ) -> None:
        super().__init__()
...
class my_ChexpertRetrieval_ZeroShotImageDataset(Dataset):
    ...
    def __init__(self,
                 datapath=f'..../CheXpert-v1.0/cheXpert8x200_img_path.csv',
                 class_names=None,
                 imgtransform=None,
                 ) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        row = self.df.iloc[index]
        img_path = r"...../datasets/chexpert/" + row.Path
        img = Image.open(img_path)
        img = self._pad_img(img)
        img = self.transform(img).unsqueeze(1)
...
"""Supervise finetuned cls"""
class my_SuperviseImageDataset_chexpert(Dataset):
    def __init__(self,
                 datapath=None,
                 class_names=None,
                 imgtransform=None,
...
class my_SuperviseImageDataset_rsna(Dataset):
    def __init__(self,
                 datapath=None,
                 class_names=None,
                 imgtransform=None,
...
class my_SuperviseImageDataset_siim(Dataset):
    def __init__(self,
                 datapath=None,
                 class_names=None,
                 imgtransform=None,
